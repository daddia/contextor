...
class IntelligenceAnalyzer:
    ...
    def analyze(
        ...
        for mdc_path in files_to_analyze:
            try:
                ...
                logger.error("Failed to analyze document", path=str(mdc_path), error=str(e))
        ...
        if "cross-linking" in features or "duplicate-detection" in features:
            try:
                ...
                logger.info("Cross-document analysis complete", document_count=len(documents))
            except Exception as e:
        ...
        for doc_data in documents:
            try:
                ...
                logger.error("Failed to update document", 
                           slug=doc_data["slug"], error=str(e))
                analysis_results["errors"] += 1
    ...
    def _filter_files_for_analysis(
        ...
        for mdc_path in mdc_files:
            try:
                ...
                previous_hash = self.previous_state.get(relative_path, {}).get("content_hash")
                ...
                logger.warning("Failed to check file hash", path=str(mdc_path), error=str(e))
    ...
    def _analyze_document(
        ...
        try:
            ...
            if "cross-linking" in features or "duplicate-detection" in features:
                fingerprint = self.similarity_analyzer.generate_fingerprint(post.content)
                doc_data["intelligence"]["content_fingerprint"] = fingerprint
                
            doc_data["intelligence"]["last_analyzed"] = datetime.utcnow().isoformat() + "Z"
        ...
        try:
            ...
            for mdc_path in analyzed_files:
                try:
                    ...
                    logger.warning("Failed to hash file for state", 
                                 path=str(mdc_path), error=str(e))
