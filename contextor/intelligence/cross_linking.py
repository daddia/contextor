...
class CrossLinker:
    ...
    def _determine_relationship_type(
        ...
        if doc1_topics and doc2_topics:
            overlap_ratio = len(doc1_topics & doc2_topics) / len(doc1_topics | doc2_topics)
            if overlap_ratio > 0.6:
                return "related-content"
                
        # Check for implementation vs concept relationship
        if any(topic in ["implementation", "example", "tutorial"] for topic in doc1_topics):
            if any(topic in ["concept", "overview", "introduction"] for topic in doc2_topics):
    ...
    def _calculate_fingerprint_similarity(
        ...
        matches = sum(1 for c1, c2 in zip(fingerprint1, fingerprint2) if c1 == c2)
