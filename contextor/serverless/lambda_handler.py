    ...
    try:
        ...
        if path == "/mcp/tools" and http_method == "GET":
            ...
            return {
                ...
                "body": json.dumps({"tools": CONTEXTOR_TOOLS, "version": "0.1.0"}),
            }
...
def list_resources() -> list:
    ...
    if context_dir.exists():
        resources.append(
            {
                "uri": f's3://{os.environ.get("STORAGE_BUCKET", "contextor-storage")}/context',
...
def async_lambda_handler(event: dict[str, Any], context: Any) -> dict[str, Any]:
    ...
    try:
        ...
        if (
            event.get("path", "").startswith("/mcp/tools/")
